...
const HomeComponent = () => {
  ...
  const loginUser = async (uid) => {
    try {
      ...
      if (response.ok) {
        ...
        localStorage.setItem('tokenTimestamp', Date.now().toString()); // Store the timestamp as a string
        setIsLoggedIn(true);
      } else {
  ...
  useEffect(() => {
    ...
    // Don't login again if you have a valid token
    if (localStorage.getItem('tokenTimestamp')) {
      const currentTimestamp = Date.now();
      const timeDifference = currentTimestamp - parseInt(localStorage.getItem('tokenTimestamp'), 10);
